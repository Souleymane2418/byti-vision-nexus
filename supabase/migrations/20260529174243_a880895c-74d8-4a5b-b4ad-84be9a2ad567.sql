...
CREATE OR REPLACE FUNCTION public.validate_order_stock(_order_id uuid, _validator_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF _validator_id IS NULL OR NOT public.is_staff(_validator_id) THEN
...
REVOKE ALL ON FUNCTION public.validate_order_stock(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.validate_order_stock(uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.validate_order_stock(uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.validate_order_stock(uuid, uuid) TO service_role;